...
app.controller("bookCtrl", function ($scope, $http) {
    ...
    $scope.getAllBook = function () {
        $http({
            url: 'http://localhost:8080/api/v1/book?page=' + $scope.pageable.pageNumber + '&size=5',
            method: 'GET'
    ...
    $scope.deleteBookByIdRequest = function (bookId) {
        $http({
            url: 'http://localhost:8080/api/v1/book' + bookId,
            method: 'DELETE'
        }).then(function () {
            $scope.getAllBook();
        })
    ...
    $scope.saveBook = function () {
        ...
        if ($scope.editFields.id === 0) {
            request = 'http://localhost:8080/api/v1/book';
            method = 'POST';
            data = {
            ...
            request = 'http://localhost:8080/api/v1/book' + $scope.editFields.id;
            method = 'PUT';
            data = {
